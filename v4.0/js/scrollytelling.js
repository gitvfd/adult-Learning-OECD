...
function  renderStory(alignment,financing,flexiguidance,inclusiveness,participation,quality,urgency){
	if(screenWidth>600){
		...
		var challengesGuideEnd = challengesChart.append("text")
		  	.attr("x",0)
		    .attr("y",0)
		    .append("tspan")
		  	.attr("id","chartGuideChallengesEnd")
		...
		var responsesGuideEnd = responsesChart.append("text")
		  	.attr("x",0)
		    .attr("y",0)
		    .append("tspan")
		  	.attr("id","chartGuideResponsesEnd")
		  	.attr("x",screenWidth - margin/2)
		    .attr("y",heightChart-margin/5)
		    .html("")
		    .style("text-anchor","end")
		...
		var dataPosChallenges=[urgency,participation,inclusiveness]
	  	var colorChallenges=["urgency","participation","inclusiveness"]
	  	var dataPosResponses=[financing,alignment,quality,flexiguidance]
	  	var colorResponses=["financing","alignment","quality","flexiguidance"]

		var titleChallenges = ["How urgent is the need to update the adult learning system ?", "To what extent are people and firms engaged in learning ?","How inclusive are adult learning opportunities ?"]
		var titleResponses = ["Is adult learning aligned with labour market needs ?", "How well is the adult learning system financed ?", "Are adult learning opportunities flexible and is information and guidance readily available ?","What are the outcomes of adult learning ? "]

		var guideChallenges=[["← less urgent","More urgent  →" ] ,["← lowest coverage","Highest coverage →" ] ,["← less inclusive","More inclusive →" ] ]
		var guideResponses=[["← least supportive","Most supportive →" ] ,["← least aligned","Most aligned →" ] ,["← lowest quality","Highest quality →" ] ,["← least adequate financing","Most adequate financing →" ] ]

		///Scrolly Challenges
		var gs = d3.graphScroll()
		    .container(d3.select('.container-1'))
		    .graph(d3.selectAll('#challengesChart'))
		    ...
		    .on('active', function(i){
				...
				challengesChart.select("#chartGuideChallenges")
		    			.html(guideChallenges[i][0]);
				challengesChart.select("#chartGuideChallengesEnd")
						.html(guideChallenges[i][1]);

				document.getElementById("challengesTitle").innerHTML = titleChallenges[i];         
		      })
		...
		var gs2 = d3.graphScroll()
		    ...
		    .on('active', function(i){
				...
				responsesChart.select("#chartGuideResponses")
		    			.html(guideResponses[i][0]);   
				responsesChart.select("#chartGuideResponsesEnd")
						.html(guideResponses[i][1]);     

				document.getElementById("responsesTitle").innerHTML = titleResponses[i];        
		      })
		}
	else{
